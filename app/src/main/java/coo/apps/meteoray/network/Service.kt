package coo.apps.meteoray.network
...
import coo.apps.meteoray.network.ServiceException.JsonDesiriazion
import coo.apps.meteoray.network.request.BaseRequest
    ...
    suspend inline fun <reified T> doRequest(request: BaseRequest): NetworkResponse {
        return withContext(Dispatchers.IO) {
            ...
            localResponse = if (request.method == BaseRequest.Method.GET) {
                when {
                    request.queryParameter.isNullOrEmpty() -> Fuel.get(
            ...
            try {
                ...
                val exception = JsonDesiriazion("Deserialzed error at ${T::class.java.name}", e)
                return@withContext NetworkResponse.Error(exception)
            }
        }
    }
